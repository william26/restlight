...
module.exports = function () {
    var prompts = [
        ...
        {
            name: 'crud',
            message: 'Do you want to generate a basic CRUD controller for that model?',
            type: 'confirm'
        },
        {
            name: 'crud_scaffold',
            message: 'Do you want to scaffold that controller?',
            type: 'confirm',
            when: function (a) {
                return a.crud;
            }
    ...
    inquirer.prompt(prompts, function (answers) {
        ...
        if (answers.crud) {
            ...
            var template_name = answers.crud_scaffold ? 'filled_controller.js' : 'empty_controller.js';

            fs.readFile(path.join(__dirname, 'templates', 'controllers', template_name), 'UTF-8', function (err, data) {
                var template = handlebars.compile(data);
                var context = {
                ...
                var output = template(context);
                fs.writeFile(path.join(process.cwd(), 'controllers', answers.name + 'Controller.js'), output, function (err) {
                    if (!err) {
        ...
        if (result.controller) {
            console.log('Successfully created controller in controllers/' + answers.name + 'Controller.js');
        }
    });
};